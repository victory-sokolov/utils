...
export const getStatusCode = (reasonPhrase: string): StatusCodes | undefined => {
    for (const [key, value] of Object.entries(ReasonPhrases)) {
...
export const getReasonPhrase = (statusCode: number): string | undefined => {
    for (const [key, value] of Object.entries(StatusCodes)) {
        if (value === statusCode) {
            return ReasonPhrases[key as keyof typeof ReasonPhrases];
        }
    }
};